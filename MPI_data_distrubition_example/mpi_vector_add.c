...
#include <stdlib.h>
#include <mpi.h>
#define BLOCK_SIZE 2 //Modified: block_size static size of 2
...
void Read_vector(
   ...
   double* a = NULL;
   double* b = NULL; // Modified: need this for new array
   int i;
   int j, k, l;  // Modified: need to declare new variables
   int local_ok = 1;
   ...
   if (my_rank == 0) {
      ...
      // Modified: need to allocate memory dynamically for b array
      ...
      /////////// Modified ////////////
      // Create a new array and store the elements to the array
      // in block-cyclic partitioning. Then invoke the scatter function
      // and send data to all processes accordingly. 
      k = 0;
      l = 0;
      for (k = 0; k < local_n/BLOCK_SIZE; k++)
      {
        for (i = 0; i < n/local_n; i++)
        {
          for (j = 0; j < BLOCK_SIZE; j++)
          {
            b[i*local_n + j + k*BLOCK_SIZE] = a[l];
            l++;
          }
        }
      }
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      MPI_Gather(local_b, local_n, MPI_DOUBLE, b, local_n, MPI_DOUBLE,
            0, comm);
      ///////// Modified /////////
      // Translate the block-cyclic back 
      k = 0;
      l = 0;
      for (k = 0; k < local_n/BLOCK_SIZE; k++)
      {
        for (i = 0; i < n/local_n; i++)
        {
          for (j = 0; j < BLOCK_SIZE; j++)
          {
            a[l] = b[i*local_n + j + k*BLOCK_SIZE];
            l++;
          }
        }
      }
